...
--IMAGE BODY--
gui:style({frame1, body})
imageBody = gui:create("frame", {text = ""})

gui:style({button2, {font = 20, y = 10, width = 200, height = 50}})
imageBody.insertImage = gui:create("button", {text = "Insert Image",  x = 10, func = function()
    editor.tool = "insert image"
end})
function imageBody.insertImage:customUpdate(self, dt)
    if editor.tool == "insert image" then
        self.color = self.hoverColor
    end
end
imageBody.deleteImage = gui:create("button", {text = "Delete Image",  x = 220, func = function()
    editor.tool = "delete image"
end})
function imageBody.deleteImage:customUpdate(self, dt)
    if editor.tool == "delete image" then
        self.color = self.hoverColor
    end
end
imageBody.idDisplay = gui:create("frame", {text = "1", x = 430, width = 100})
function imageBody.idDisplay:customUpdate(self, dt)
    self.text = editor.imageSelect
end
imageBody.newImage = gui:create("button", {text = "+", alignmentX = "right", x = -10, y = 10, width = 50, height = 50, func = function()
    menu = newImage
end})

gui:style({frame2})
imageBody.list = gui:create("frame", {text = "", x = 10, y = 70, width = 580, height = 740})

gui:style({button2, {alignmentY = "bottom", font = 18}})
imageBody.scale = gui:create("frame", {text = "Scale", x = 10, y = -10})
imageBody.scaleInput = gui:create("input", {text = "1", x = 120, y = -10, func = function(self)
    editor.imageScale = tonumber(self.text)
end})
imageBody.multiple = gui:create("frame", {text = "Multiple", x = 230, y = -10})
imageBody.multipleInput = gui:create("input", {text = "16", x = 340, y = -10, func = function(self)
    editor.imageMultiple = tonumber(self.text)
end})
imageBody.decal = gui:create("frame", {text = "bad input may cause errors", x = 450, y = -10, width = 140, font = 15})
gui:style({frame1})
imageBody.delete = gui:create("button", {hoverColor = {1, 0, 0}, text = "", alignmentX = "right", alignmentY = "bottom", width = 10, height = 10, func = function()
    if editor.images[editor.imageSelect] ~= nil then
        table.remove(editor.images, editor.imageSelect)
...
function updateImageList(images)
    gui:style({frame2})
    local list = gui:create("frame", {text = "", x = 10, y = 70, width = 580, height = 740})

    gui:style({button2, {width = 570, height = 40}})
    for i, image in ipairs(images) do
        local button = gui:create("button", {text = tostring(i), x = 5, y = 45 * (i - 1) + 5, textAlignmentX = "left", textMargin = 20, font = 18, func = function(self)
            editor.imageSelect = tonumber(self.text)
        end})
        function button:customUpdate(self, dt)
            if tonumber(self.text) == editor.imageSelect and editor.tool == "insert image" then
                self.color = self.hoverColor
            end
        end
        button.imageName = gui:create("frame", {text = image.name, x = 0, y = 0, color = {0, 0, 0, 0}, font = 18})

        list["image button "..i] = button
    end
